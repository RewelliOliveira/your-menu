    ...
    <div className="relative w-full h-60 bg-black text-white flex items-center justify-center">
      ...
      <div className="absolute top-2 right-2 z-20 flex items-center gap-2">
        <button
          ...
          className={`w-12 h-6 flex items-center rounded-full p-1 duration-300 ${data.isOpen ? 'bg-green-500' : 'bg-gray-400'
            }`}
        >
          <div
            className={`bg-white w-4 h-4 rounded-full shadow-md transform duration-300 ${data.isOpen ? 'translate-x-6' : ''
              }`}
          />
        </button>
        ...
        <div className="flex gap-6 items-center text-sm mt-2">
          <div className="flex items-center gap-1 min-w-[90px]">
            <img
              ...
              className="w-5 h-5"
            />
            <span className="inline-block min-w-[60px]">{data.isOpen ? 'Aberto' : 'Fechado'}</span>
          </div>
